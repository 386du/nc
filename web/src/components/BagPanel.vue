...
function getBagGroupKey(item: any) {
  return String(item?.bagGroupKey || `${Number(item?.id || 0)}:normal`)
}

function handleSellClick(item: any) {
  if (batchMode.value) {
    const key = getBagGroupKey(item)
...
async function handleConfirm() {
  ...
  try {
    if (action === 'sell' && item) {
      const itemKey = getBagGroupKey(item)
      const sellItems = originalItems.value
        .filter((it: any) => getBagGroupKey(it) === itemKey)
      ...
      const selectedKeys = new Set(selectedItems.map((si: any) => getBagGroupKey(si)))
      const itemsToSell = originalItems.value
        .filter((it: any) => selectedKeys.has(getBagGroupKey(it)))
        .map((it: any) => ({ id: it.id, count: it.count, uid: it.uid || 0 }))
      ...
      if (res.ok) {
        ...
        for (const si of selectedItems) {
          const fi = filteredItems.value.find((f: any) => getBagGroupKey(f) === getBagGroupKey(si))
          if (fi) {
...
function selectAllSellable() {
  ...
  for (const item of filteredItems.value) {
    if (canBatchSell(item)) {
      selectedForBatch.value.add(getBagGroupKey(item))
    }
  }
}
...
function handleBatchSellClick() {
  ...
  const selectedKeys = Array.from(selectedForBatch.value)
  if (selectedKeys.length === 0) {
    toastStore.warning('请先选择要出售的物品')
    return
  }
  const selectedDisplayItems = filteredItems.value.filter((item: any) => selectedKeys.includes(getBagGroupKey(item)))
  ...
  const messages = [
    `确定要批量出售选中的 ${selectedDisplayItems.length} 种物品吗?`,
  ]
  if (totalGold > 0) {
    ...
    <div v-else>
      ...
      <div class="grid grid-cols-2 gap-4 lg:grid-cols-5 md:grid-cols-4 sm:grid-cols-3 xl:grid-cols-6">
        <div
          ...
          :class="{
            'ring-2 ring-orange-500 dark:ring-orange-400': batchMode && selectedForBatch.has(getBagGroupKey(item)),
            'opacity-50': batchMode && canBatchSell(item) && !selectedForBatch.has(getBagGroupKey(item)),
          }"
        ...
        >
          ...
          <div class="absolute right-1 top-1 flex gap-1">
            ...
            <div
              ...
              :class="selectedForBatch.has(getBagGroupKey(item))
                ? 'border-orange-500 bg-orange-500 text-white'
                : 'border-gray-300 bg-white dark:border-gray-600 dark:bg-gray-700'"
            >
              <div v-if="selectedForBatch.has(getBagGroupKey(item))" class="i-carbon-checkmark text-xs" />
            </div>
          </div>
          ...
          <div v-if="item.isMutantItem && item.mutantTypes && item.mutantTypes.length > 0" class="mb-1 flex flex-wrap justify-center gap-1">
            <span v-for="mt in item.mutantTypes" :key="mt" class="inline-block rounded-full bg-gradient-to-r from-purple-500 to-pink-500 px-2 py-0.5 text-[10px] font-bold text-white shadow-sm">
              🌟 {{ mt }}
            </span>
          </div>
