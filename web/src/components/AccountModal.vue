...
// 手动提交前尝试通过 code 查询 QQ 号
async function resolveUinFromCode(code: string) {
  try {
    const res = await api.post('/api/qr/check', { code: code.trim() })
    if (res.data.ok && res.data.data && res.data.data.uin) {
      return {
...
async function submitManual() {
  ...
  // 尝试通过 code 查询 QQ 号
  const uinInfo = await resolveUinFromCode(code)
  ...
  if (props.editData) {
    ...
    if (onlyNameChanged) {
      ...
      payload = {
        ...
        loginType: 'manual',
        uin: uinInfo?.uin || undefined,
        qq: uinInfo?.uin || undefined,
        avatar: uinInfo?.avatar || undefined,
      }
    }
  }
  else {
    payload = {
      name: form.name || (uinInfo?.nickname || ''),
      code,
      platform: form.platform,
      loginType: 'manual',
      uin: uinInfo?.uin || undefined,
