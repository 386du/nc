...
function getStats(statusData, userState, connected, limits) {
    ...
    const rawDiamond = (userObj.diamond !== null && userObj.diamond !== undefined) ? userObj.diamond : statusObj.diamond;
    const currentDiamond = Number.isFinite(Number(rawDiamond)) ? Number(rawDiamond) : 0;
    ...
    return {
        ...
        status: {
            ...
            uid: userObj.gid || statusObj.uid || '',
            gid: userObj.gid || statusObj.gid || 0,
            uin: userObj.uin || statusObj.uin || '',
            diamond: Number.isFinite(Number(userObj.diamond)) ? Number(userObj.diamond) : (Number.isFinite(Number(statusObj.diamond)) ? Number(statusObj.diamond) : 0),
        },
