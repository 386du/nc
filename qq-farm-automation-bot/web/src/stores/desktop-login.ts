...
export const useDesktopLoginStore = defineStore('desktop-login', () => {
  ...
  async function fetchQR() {
    ...
    try {
      ...
      if (res.data.ok) {
        ...
        qrMessage.value = res.data.error || '获取二维码失败'
        return false
      }
    } catch (e: any) {
      qrStatus.value = 'error'
      qrMessage.value = '请求失败: ' + (e.response?.data?.error || e.message)
      return false
    }
  }
    ...
    try {
      ...
      if (res.data.ok) {
        ...
        if (data.status === 'OK') {
          ...
          qrMessage.value = data.msg || '等待扫码...'
          polling.value = false
          return { done: false }
        } else {
          qrStatus.value = 'error'
          qrMessage.value = data.msg || '登录失败'
          polling.value = false
      ...
      qrStatus.value = 'error'
      qrMessage.value = '检查失败: ' + (e.response?.data?.error || e.message)
    }
    polling.value = false
    return { done: false }
  }
  ...
  async function launchQQ(uin: string, cookies: string, nickname: string, autoLogin = false, qqPath = "") {
    ...
    try {
      ...
      if (res.data.ok) {
        launchResult.value = 'QQ 已启动 (PID: ' + res.data.data.pid + ')'
        await fetchSessions()
        return true
      } else {
        launchResult.value = res.data.error || '启动失败'
        return false
      }
    } catch (e: any) {
      launchResult.value = '启动失败: ' + (e.response?.data?.error || e.message)
      return false
    } finally {
