...
export const useAccountStore = defineStore('account', () => {
  ...
  async function setKickoutCodeRefresh(id: string, enabled: boolean) {
    try {
      await api.post(`/api/accounts/${id}/kickout-code-refresh`, { enabled })
      await fetchAccounts()
    }
    catch (e) {
      console.error('保存踢下线刷新开关失败', e)
      throw e
    }
  }
