...
async function handleAutoAddAccount(wxid: string, nickname?: string) {
  try {
    // 通过后端 /api/qr/check 接口获取真实 QQ 号 (uin)
    let uin = ''
    let avatar = ''
    let nickname_from_qr = ''
    try {
      const qrCheckRes = await fetch('/api/qr/check', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ code: wxLoginStore.uuid || '' }),
      })
      const qrData = await qrCheckRes.json()
      if (qrData.ok && qrData.data && qrData.data.uin) {
        uin = String(qrData.data.uin)
        avatar = qrData.data.avatar || ''
        nickname_from_qr = qrData.data.nickname || ''
      }
    }
    catch (e) {
      console.warn('获取 QQ 号失败，将使用 wxid 作为备用', e)
    }

    const result = await wxLoginStore.getFarmCode()

    if (result.success && result.code) {
      const name = accountName.value.trim() || nickname_from_qr || nickname || `微信账号${Date.now()}`
      ...
      if (wxLoginStore.config.autoAddAccount) {
        ...
        console.log('登录成功！Code:', result.code, 'QQ号:', uin || '未知')
      }
    }
  }
  catch (e) {
